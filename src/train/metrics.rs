...
pub(super) struct CsvRow {
    ...
    /// Average principal variation depth (most-visited path from root,
    /// edges = actions + chance nodes) across all searches this iteration.
    #[serde(serialize_with = "round6::f32")]
...
pub(super) fn compute_iter_stats(samples: &[&Sample]) -> IterStats {
    ...
    let sum_q: f64 = samples.iter().map(|s| s.q_wdl.q() as f64).sum();
    let mean_z = sum_z / n;
    let mean_q = sum_q / n;
    ...
    let var_q: f64 = samples
        .iter()
        .map(|s| (s.q_wdl.q() as f64 - mean_q).powi(2))
        .sum::<f64>()
        / n;
    ...
    for s in samples {
        let err = (s.q_wdl.q() - s.z).abs() as f64;
        if s.game_length == 0 {
            continue;
        }
