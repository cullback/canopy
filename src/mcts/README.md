...
struct Config {
    ...
    pub gumbel_scale: f32,         // Gumbel noise scale (default 1.0; 0.0 for perfect-info games)
}
```
...
> **Evaluator-independent state machine**

The MCTS module has zero dependency on neural network code. Search yields `NeedsEval(&[G])` with borrowed leaf states; the caller evaluates them however it wants (GPU batch, rollout, remote inference) and feeds results back through `step`. This is simpler than KataGo's deeply-coupled evaluator or AlphaZero's blocking oracle calls, and naturally supports batching across multiple concurrent searches — the caller collects leaves from parallel workers into a single forward pass.

> **Minimal configuration surface**

5 parameters (`num_simulations`, `num_sampled_actions`, `c_visit`, `c_scale`, `leaf_batch_size`). Gumbel AlphaZero eliminates the need for Dirichlet noise weights, exploration temperature schedules, PUCT constants, and FPU parameters that other implementations require. Compare KataGo's 80+ search parameters.

## Key optimizations

> **Gumbel AlphaZero (replacing PUCT entirely)**

Replaces PUCT selection, Dirichlet noise, temperature-based action sampling, and visit-count policy targets with a unified Gumbel-based framework. Root uses Sequential Halving with Gumbel noise; non-root uses deterministic improved-policy selection. Guarantees policy improvement even at 2 simulations.
...
Splits the MCTS loop so that search pauses at leaf nodes, batches multiple pending evaluations into a single GPU forward pass, then resumes. `leaf_batch_size` controls how many leaves to collect before yielding.
...
| File             | Contents                                                                                                  |
| ---------------- | --------------------------------------------------------------------------------------------------------- |
| `mod.rs`         | `Search`, `Config`, `Step`, `SearchResult`, Gumbel selection, simulation loop, improved policy extraction |
| `tree.rs`        | `Tree` arena, `NodeData`, `Edge`, transposition table, `compact`, `backprop`, `recompute_q`               |
| `seq_halving.rs` | Pre-computed Sequential Halving schedule (candidate assignment table and halving points)                  |
